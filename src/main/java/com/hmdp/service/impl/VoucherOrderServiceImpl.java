...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    @PostConstruct // 表示当前类初始化之后立即执行
    private void init() {
        // 没有创建消息队列前，就将此行注释，否则程序运行报错
        // 用一个线程不断地从消息队列中获取订单信息，创建订单
    ...
    /*
    // v4.0 秒杀优化：异步下单(先将库存判断和一人一单校验放在redis中完成), 使用seckill的lua脚本保证原子性
    // 阻塞队列，用来存放下单请求
    private BlockingQueue<VoucherOrder> orderTasks = new ArrayBlockingQueue<>(1024 * 1024);

    // v4.0 使用jvm的阻塞队列来实现异步秒杀（存在内存限制和数据安全问题）
    private class VoucherOrderHandler implements Runnable {
    ...
    public Result seckillVoucher(Long voucherId) {
        ...
        // // 5.一人一单
        // Long userId = UserHolder.getUser().getId();
        // // 事务提交之后，再去释放锁，这样才不会发生并发安全问题
        // // 使用synchronized锁当前用户（userid值一样时），这样不同的用户就会有不一样的锁，这样锁的粒度更小
        // // 集群环境下不能保证一人一单，因为不同的jvm有不同的锁监视器，导致一个用户开多个线程可以获取到多个锁
    ...
    /*
    // v2.0 利用redis本身的setnx命令实现分布式锁
    @Transactional
    public Result createVoucherOrder(Long voucherId) {
        // 5.一人一单
        Long userId = UserHolder.getUser().getId();

        // 创建锁对象（SimpleRedisLock：自己写的加锁、解锁）
    ...
    /*
    // v3.0 利用redisson实现分布式锁
    @Transactional
    public Result createVoucherOrder(Long voucherId) {
        // 5.一人一单
        Long userId = UserHolder.getUser().getId();

        // 利用RedissonClient创建锁对象
        RLock redisLock = redissonClient.getLock("lock:order:" + userId);
        // 尝试获取锁
        boolean isLock = redisLock.tryLock();
        // 判断
